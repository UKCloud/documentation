...
toc_sub3:
toc_sub4:
toc_title: Use TrilioVault for self-service backup
...
If you are currently using a SSO enabled account to access the OpenStack platform, and you wish to make use of the TrilioVault service, please raise a support ticket and request a Keystone account for Trilio managment.
...
2. Select the desired values as explained in the following table:
    ...
    Full Backup Interval | Frequency at which Full Backup should be taken. The options are:<ul><li>Never (The default value selected is Never).</li><li>Always (Value 0 means Always).</li><li>No. of Days (Number of incremental snapshots to take full backup between 1 and 999). If this option is selected, enter the number provided in the text box.</li></ul>
...
2. Click _**WORKLOAD NAME**_. The Workload Details screen is displayed as shown in Figure 8.
    ...
    **Table 5: Workload Details Description.**
    ...
    Policy    | Displays information of the backup schedule and retention policy. Details such as Scheduler Enabled, Start Date, Start Time, End Date, Repeat every, Next snapshot runs at, Full Backup interval can be seen here. It also displays Number of snapshots to keep. Refer to Figure 11 for details.
...
4. To mount a snapshot, login to TrilioVault and Navigate to Backups. Click _**WORKLOAD NAME**_, then click Snapshots tab.
...
8. Access File Manager using _**FLOATING IP**_ created in step 3. Refer to Figure 28 for details.
...
2. Login to Horizon dashboard and navigate to Backups. Click _**WORKLOAD NAME**_, and then click Snapshots tab.
...
4. Navigate to the Restore tab and click _**RESTORE NAME**_. The Restore Details screen as shown in Figure 48 is displayed.
