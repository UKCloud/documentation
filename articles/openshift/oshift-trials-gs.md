...
OpenShift is deployed with an internal image registry making use of UKCloud's object storage service for persistence.  This includes 60GB of free storage capacity which does not come out of a customer's 2TB block storage capacity, rather this is in addition to the 2TB data limit. UKCloud do provision ElasticSearch, Fluentd and Kibana stack which uses 20GB, Hawkular Metrics backed by a Cassandra database which uses 10GB and the Prometheus Operator which uses 106GB of tier 2 storage (which consumes some of the 2TB storage limit).
