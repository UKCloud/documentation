...
UKCloud's environment is built using VMware vSphere, the most ubiquitous and mature hypervisor available. Secure multi-tenancy is achieved with vCloud Director/VMware Cloud Director.
...
Your UKCloud-provided dedicated virtual firewall is managed exclusively by you. You can set and manage firewall rules via the vCloud Director/VMware Cloud Director Tenant Portal and vCloud API.
...
You can quickly and easily allocate additional storage via the self-service vCloud Director/VMware Cloud Director Tenant Portal or vCloud API. Additional storage is charged on a per-GiB basis as listed in the [UKCloud Pricing Guide](https://ukcloud.com/pricing-guide).
...
You control a VM via the vCloud Director/VMware Cloud Director Tenant Portal or vCloud API. Controls include stop, start, restart, load media, clone, snapshot, and so on.
...
Yes, you can do this through the vCloud Director/VMware Cloud Director Tenant Portal. This operation can also be performed programmatically via the vCloud API.
...
You can create snapshots using the vCloud Director/VMware Cloud Director Tenant Portal and these are stored in the same zone as the source VM. This is not to be confused with the UKCloud Snapshot Protection service, which uses a temporary snapshot to backup the VM to an external system in another UKCloud site.

For further reference, [*Virtual machine data recovery options*](vmw-ref-vm-data-recovery.md) explains the different recovery options available to you through vCloud Director/VMware Cloud Director.
