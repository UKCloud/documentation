...
With Anti-Virus as a Service provides you with the confidence that your environments are protected from the dangers of viruses and malware, all without the usual headaches associated with the deployment, management, and operation of an anti-virus solution.

### What technologies do you use?

Built-upon the capabilities of our [Managed Monitoring as a Service](https://docs.ukcloud.com/articles/managed-services/man-monitoring-sco.html), UKCloud uses the industry leading endpoint security platform [SentinelOne](https://www.sentinelone.com/) as the engine for this service, harnessing the power of machine learning to providing protection which goes beyond that of traditional anti-virus or anti-malware solutions.

### What actions will UKCloud perform upon positive detection?
Our service has the concept of the following two detection types

Detection Type | Description                                                                      | Default Actions
---------------|----------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------
Malicious      | SentinelOne has detected a file or process as being harmful                      | <ul><li>Quarantine the affected file </li></ul><ul><li> Notify the customer</li></ul>
Suspicious     | SentinelOne has detected a file or process as having the potential to be harmful | <ul><li>Notify the customer </li></ul><ul><li> Take no action on the affected file</li></ul>
...
We envisage that there should not be a scenario which prevents UKCloud in meeting the Anti-virus as a Service platform availability KPI defined within the [Service Scope](https://docs.ukcloud.com/articles/managed-services/man-antivirus-sco.html) for this service. Current Service Status and past past incident reports are published on the [Status Page](https://status.ukcloud.com/).
...
* Prevention - Static AI on the endpoint prevents attacks in-line in real time. Consistently ranked for highest efficacy and lowest false-positives, SentinelOne’s static AI model replaces
legacy antivirus.
...
### How do I access my Anti-Virus environment?
...
Currently there is no way for a customer to directly manage this service as it is assumed UKCloud will manage all anti-virus aspects as part of this service.

Any changes to the service for items such as file or folder exclusions should be requested initially by raising a ticket within the [My Calls](https://portal.skyscapecloud.com/support/ivanti) section of the Portal. Alternatively, you can contact support by phone or email.

### How do I instantiate a manual or scheduled scan?
Unlike traditional anti-virus solutions, SentinelOne's real-time AI threat recognition capabilities prevents viruses from embedding themselves in to systems, removing the traditional concept of ad-hock full system scans.

### Do I get access to a dashboard or reports?

As it is assumed UKCloud will manage all anti-virus aspects as part of this service, there is currently no dashboard or reporting features for customers to access.
...
UKCloud will continue to charge for any resources that you have elected to have anti-virus protection regardless of the state they're in. In order to stop any charges, you'll need to raise a ticket within the [My Calls](https://portal.skyscapecloud.com/support/ivanti) section of the Portal. Alternatively, you can contact support by phone or email.
...
### Does an agent need to be installed on my endpoints to enable this service?
Yes, the SentinelOne agent will need to be deployed to each endpoint, and runs autonomously on each device, without reliance on an internet connection. The agent sits at the kernel level and monitors all processes in real time. This process is performed by the agents Dynamic Behavioral Tracking engine, and allows users to see exactly what happened on an endpoint at each stage of execution. This includes origin, patient zero, process and file activity, registry event, network connections, and forensic data.
